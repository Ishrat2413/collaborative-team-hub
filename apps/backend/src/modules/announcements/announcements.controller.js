...
import { AUDIT_ACTION, SOCKET_EVENTS, AVAILABLE_REACTIONS } from '@team-hub/shared';
...
export const createAnnouncement = asyncHandler(async (req, res) => {
  ...
  const announcement = await announcementsService.createAnnouncement(workspaceId, req.user.id, data);
  ...
  emitToWorkspace(workspaceId, SOCKET_EVENTS.ANNOUNCEMENT_CREATED, announcement);
...
export const getAnnouncements = asyncHandler(async (req, res) => {
  ...
  const { announcements, total } = await announcementsService.getAnnouncements(workspaceId, {
    page: parseInt(page),
...
export const getAnnouncement = asyncHandler(async (req, res) => {
  const announcement = await announcementsService.getAnnouncement(req.params.announcementId);
  sendSuccess(res, 200, 'Announcement retrieved.', { announcement });
});

/** PATCH /api/v1/announcements/:announcementId/pin */
export const togglePin = asyncHandler(async (req, res) => {
  const announcement = await announcementsService.togglePin(req.params.announcementId);

  await createAuditLog({
    workspaceId: req.body.workspaceId,
    actorId: req.user.id,
    action: announcement.isPinned ? AUDIT_ACTION.ANNOUNCEMENT_PINNED : AUDIT_ACTION.ANNOUNCEMENT_UNPINNED,
    entityType: 'Announcement',
    entityId: announcement.id,
    entityTitle: announcement.title,
  });

  emitToWorkspace(req.body.workspaceId, SOCKET_EVENTS.ANNOUNCEMENT_PINNED, announcement);
  sendSuccess(res, 200, `Announcement ${announcement.isPinned ? 'pinned' : 'unpinned'}.`, { announcement });
});

/** POST /api/v1/announcements/:announcementId/reactions */
export const toggleReaction = asyncHandler(async (req, res) => {
  const { emoji, workspaceId } = z.object({
