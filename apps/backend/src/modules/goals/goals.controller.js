...
const goalSchema = z.object({
  ...
  status: z.enum(['NOT_STARTED', 'IN_PROGRESS', 'COMPLETED', 'AT_RISK', 'CANCELLED']).optional(),
...
export const getGoal = asyncHandler(async (req, res) => {
  const goal = await goalsService.getGoal(req.params.goalId, req.query.workspaceId);
...
export const updateGoal = asyncHandler(async (req, res) => {
  ...
  const prevGoal = await goalsService.getGoal(req.params.goalId, req.body.workspaceId || req.query.workspaceId);
  ...
  if (data.status && data.status !== prevGoal.status) {
    ...
    emitToWorkspace(prevGoal.workspaceId, SOCKET_EVENTS.GOAL_STATUS_CHANGED, goal);
  } else {
...
export const addActivity = asyncHandler(async (req, res) => {
  ...
  const update = await goalsService.addActivityUpdate(req.params.goalId, req.user.id, content);
