...
async function main() {
  ...
  const users = await Promise.all([
    prisma.user.create({
      data: {
        ...
        avatarUrl: "https://api.dicebear.com/8.x/avataaars/svg?seed=ishrat",
      },
    }),
    prisma.user.create({
      data: {
  ...
  const [ishrat, sam, jordan, morgan] = users;
  ...
  const productWorkspace = await prisma.workspace.create({
    data: {
      name: "Product Team",
      description:
        "Where great products are born. Q3 launch planning and execution.",
      accentColor: "#6366f1",
    },
  });
  ...
  await prisma.workspaceMember.createMany({
    data: [
      // Product Team
      { userId: ishrat.id, workspaceId: productWorkspace.id, role: "ADMIN" },
      ...
      { userId: ishrat.id, workspaceId: marketingWorkspace.id, role: "MEMBER" },
  ...
  const goals = await Promise.all([
    prisma.goal.create({
      data: {
        ...
        description:
          "Complete redesign and rebuild of the core product with new features, improved performance, and a polished UI.",
        status: "IN_PROGRESS",
        progress: 65,
    ...
    prisma.goal.create({
      data: {
        ...
        description:
          "Reduce average API response time by 40% through query optimization, caching, and infrastructure improvements.",
        status: "IN_PROGRESS",
        progress: 80,
    ...
    prisma.goal.create({
      data: {
        ...
        description:
          "Create a frictionless onboarding experience that gets users to their first aha moment within 5 minutes.",
        status: "NOT_STARTED",
        progress: 0,
    ...
    prisma.goal.create({
      data: {
        ...
        description:
          "Ship the first beta version of the iOS and Android companion app.",
        status: "COMPLETED",
        progress: 100,
    ...
    prisma.goal.create({
      data: {
        ...
        description:
          "Execute a multi-channel brand awareness campaign targeting our core demographic.",
        status: "IN_PROGRESS",
        progress: 45,
    ...
    prisma.goal.create({
      data: {
        ...
        description:
          "Develop and execute a content strategy to drive organic traffic by 50%.",
        status: "AT_RISK",
        progress: 20,
  ...
  await prisma.milestone.createMany({
    data: [
      ...
      {
        goalId: platformGoal.id,
        title: "Frontend components built",
        progress: 80,
        completed: false,
      },
      {
      ...
      {
        goalId: apiGoal.id,
        title: "Query analysis & N+1 fixes",
        progress: 100,
        completed: true,
      },
      {
      ...
      {
        goalId: brandGoal.id,
        title: "Ad creatives designed",
        progress: 60,
        completed: false,
      },
      {
        goalId: brandGoal.id,
        title: "Channels booked",
        progress: 0,
        completed: false,
      },
    ],
    ...
    data: [
      {
        ...
        authorId: ishrat.id,
        content:
          "Completed the backend API for the new workspace module. All endpoints tested and documented.",
      },
      {
        goalId: platformGoal.id,
        authorId: sam.id,
        content:
          "Frontend component library is 80% complete. Starting on the dashboard views next.",
      },
      {
        goalId: platformGoal.id,
        authorId: jordan.id,
        content:
          "Design tokens and component specs finalized. Handed off to engineering.",
      },
      {
        goalId: apiGoal.id,
        authorId: sam.id,
        content:
          "Identified 12 N+1 query issues. Fixed 10 of them — seeing 35% improvement in benchmarks already!",
      },
      {
        goalId: apiGoal.id,
        authorId: sam.id,
        content:
          "Redis caching layer is live in staging. Response times down to avg 45ms from 180ms.",
      },
    ],
  });
  ...
  const announcements = await Promise.all([
    prisma.announcement.create({
      data: {
        ...
        content:
          "<p>Hey team! We had an incredible kickoff session today. Here are the key decisions:</p><ul><li>We're targeting <strong>July 31st</strong> for the v2.0 release</li><li>Daily standups move to 9:30 AM starting Monday</li><li>Each squad owns their module end-to-end</li></ul><p>Let's make this happen! 💪</p>",
        isPinned: true,
      },
    }),
    prisma.announcement.create({
    ...
    prisma.announcement.create({
      data: {
        ...
        content:
          "<p>Huge milestone — our mobile beta is officially out! 🎉</p><p>Download links have been shared with the beta test group. We're collecting feedback for the next 2 weeks before the public launch.</p><p>Great work everyone — especially @jordan for the stunning UI work!</p>",
        isPinned: false,
      },
    }),
    prisma.announcement.create({
      data: {
        ...
        content:
          "<p>The Q3 brand campaign brief is ready for review. Please read through and add your comments by EOD Friday.</p><p>Key points:</p><ul><li>Target audience: Tech-forward professionals, 25-40</li><li>Primary channels: LinkedIn, YouTube, Podcast sponsorships</li><li>Budget: $45,000</li><li>KPI: 30% brand recall increase</li></ul>",
        isPinned: true,
      },
    }),
  ]);

  const [kickoffAnnouncement, apiAnnouncement, mobileAnnouncement] =
    announcements;
  ...
  await prisma.reaction.createMany({
    data: [
      ...
      { announcementId: kickoffAnnouncement.id, userId: sam.id, emoji: "🔥" },
      { announcementId: apiAnnouncement.id, userId: ishrat.id, emoji: "🎉" },
    ...
    data: [
      {
        ...
        authorId: sam.id,
        content:
          "Pumped for this! Let's get it done. @jordan the Figma specs look amazing btw.",
      },
      {
        announcementId: kickoffAnnouncement.id,
        authorId: jordan.id,
        content:
          "Thanks @sam! Happy to do a walkthrough anytime. Can we schedule a design review this week?",
      },
      {
        announcementId: apiAnnouncement.id,
        authorId: ishrat.id,
        content:
          "This is massive. 40% improvement is ahead of target. Great work @sam!",
      },
      {
        announcementId: mobileAnnouncement.id,
        authorId: jordan.id,
        content:
          "So excited to see this ship! The feedback from beta users has been phenomenal.",
      },
    ],
  });
  ...
  await prisma.actionItem.createMany({
    data: [
      // Platform goal items
      {
        ...
        description:
          "Ensure refresh tokens are rotated on each use to prevent replay attacks.",
        status: "DONE",
        priority: "HIGH",
      ...
      },
      {
        ...
        description:
          "Drag-and-drop kanban with column status updates via Socket.io.",
        status: "IN_PROGRESS",
        priority: "HIGH",
        dueDate: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000),
      },
      ...
      // API goal items
      {
        ...
        description:
          "k6 load test targeting 500 concurrent users for 10 minutes.",
        status: "TODO",
        priority: "URGENT",
  ...
  console.log("   📧 ishrat@demo.com  | 🔑 Password1  (Admin — Product Team)");
