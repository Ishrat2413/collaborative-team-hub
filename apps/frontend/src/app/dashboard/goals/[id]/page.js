...
import { Badge, ProgressBar, Spinner, Avatar } from '../../../../components/ui/index';
import { goalStatusColor, statusLabel, formatDate, timeAgo, isOverdue } from '../../../../lib/utils';
  ...
  return (
    <div className="flex items-center gap-3 py-2 border-b border-slate-100 dark:border-slate-700 last:border-0">
      <input
        ...
        onChange={(e) => onUpdate(milestone.id, { completed: e.target.checked, progress: e.target.checked ? 100 : 0 })}
        className="h-4 w-4 rounded accent-[var(--accent-color)]"
      />
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${milestone.completed ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-300'}`}>
          {milestone.title}
        </p>
      ...
      <button onClick={() => onDelete(milestone.id)} className="p-1 text-slate-300 hover:text-red-400 transition-colors">
...
export default function GoalDetailPage() {
  ...
  const { fetchGoal, selectedGoal, updateGoal, onActivityUpdate } = useGoalStore();
  ...
  if (!selectedGoal) return <p className="text-slate-500 py-10 text-center">Goal not found.</p>;
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-400">
        <Link href="/dashboard/goals" className="hover:text-slate-600 dark:hover:text-slate-300">Goals</Link>
        <span>/</span>
        <span className="text-slate-600 dark:text-slate-300 truncate max-w-xs">{goal.title}</span>
      </div>
      ...
      <div className="card">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold text-slate-900 dark:text-white">{goal.title}</h1>
            {goal.description && (
        ...
        <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 text-xs text-slate-500">
          ...
          {goal.dueDate && (
            <div className={`flex items-center gap-1 ${isOverdue(goal.dueDate) && goal.status !== 'COMPLETED' ? 'text-red-500' : ''}`}>
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <form onSubmit={handleAddMilestone} className="flex gap-2">
            ...
            />
            <button type="submit" disabled={addingMilestone} className="btn-primary shrink-0 px-3">
              {addingMilestone ? '...' : '+'}
            </button>
          </form>
        </div>
        ...
        {/* Activity Feed */}
        <div className="card">
          <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-3">Activity Feed</h2>
          ...
          <form onSubmit={handlePostActivity} className="flex gap-2 mb-4">
            ...
            />
            <button type="submit" disabled={postingActivity || !activityText.trim()} className="btn-primary shrink-0 px-3 text-sm">
              Post
            </button>
          </form>
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
            {(goal.activityFeed || []).length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">No activity yet. Post the first update!</p>
            ) : (
                ...
                <div key={a.id} className="flex gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-xs font-semibold text-slate-800 dark:text-slate-200">{a.author?.name}</span>
                      <span className="text-xs text-slate-400">{timeAgo(a.createdAt)}</span>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5 leading-relaxed">{a.content}</p>
                  </div>
                </div>
              ))
            )}
              ...
              <div key={item.id} className="flex items-center justify-between py-2.5">
                <div className="flex items-center gap-2 min-w-0">
                  <span className={`h-2 w-2 rounded-full shrink-0 ${
                    ...
                    item.status === 'IN_REVIEW' ? 'bg-amber-400' : 'bg-slate-300'
                  }`} />
