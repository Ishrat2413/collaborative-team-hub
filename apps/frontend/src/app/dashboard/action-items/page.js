...
import { Badge, Avatar, EmptyState, Spinner } from '../../../components/ui/index';
import Modal from '../../../components/ui/Modal';
import { ACTION_ITEM_STATUS, ACTION_ITEM_PRIORITY } from '@team-hub/shared';
import { statusLabel, priorityColor, formatDate, isOverdue } from '../../../lib/utils';
...
function CreateItemModal({ open, onClose, workspaceId, members, goals }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      onClose();
      setForm({ title: '', description: '', status: 'TODO', priority: 'MEDIUM', assigneeId: '', goalId: '', dueDate: '' });
    } else {
    ...
    <Modal open={open} onClose={onClose} title="New action item">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input className="input-base" required placeholder="What needs to be done?"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select className="input-base" value={form.assigneeId}
        ...
        <div>
          <label className="block text-sm font-medium mb-1.5">Link to goal</label>
          <select className="input-base" value={form.goalId}
  ...
  return (
    <div className="flex flex-col min-w-[260px] bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 h-full">
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-slate-200 dark:border-slate-700">
        ...
        <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{column.label}</span>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2">
        ...
        {items.length === 0 && (
          <div className="py-6 text-center text-xs text-slate-400">No items</div>
        )}
      </div>
    </div>
  );
...
function ListView({ items, onStatusChange }) {
  return (
    <div className="card divide-y divide-slate-100 dark:divide-slate-700 p-0 overflow-hidden">
      {items.length === 0 && (
        <p className="text-sm text-slate-400 text-center py-8">No action items yet.</p>
      )}
      {items.map((item) => {
        const overdue = isOverdue(item.dueDate) && item.status !== 'DONE';
        return (
          <div key={item.id} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50">
            <span className={`h-2 w-2 shrink-0 rounded-full ${
              item.priority === 'URGENT' ? 'bg-red-500' : item.priority === 'HIGH' ? 'bg-orange-400' :
            ...
            <div className="flex-1 min-w-0">
              ...
              {item.goal && <p className="text-xs text-slate-400">↳ {item.goal.title}</p>}
            </div>
            {item.assignee && <Avatar user={item.assignee} size="xs" />}
            {item.dueDate && (
              <span className={`text-xs shrink-0 ${overdue ? 'text-red-500' : 'text-slate-400'}`}>
...
export default function ActionItemsPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/goals?workspaceId=${activeWorkspace.id}`).then((r) => setGoals(r.data.data.goals));
  }, [activeWorkspace?.id]);
